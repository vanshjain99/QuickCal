...
async def extract_schedule(file: UploadFile = File(...)):
    ...
    system_prompt = (
        ...
        "\n"
        "At the document level, extract:\n"
        "- calendar_name: A contextual name for the schedule calendar (e.g., 'Fall 2026 - B.Tech CSE', 'Shift Chart - Q3') parsed from headers, institutional logos, or titles. If no title/header is available, guess a sensible name.\n"
        "\n"
        ...
        "7. category: A dynamic grouping category for the event. Analyze the schedule to identify underlying event types or themes (e.g., LECTURE, PRACTICAL, TUTORIAL, Seminar, or operations shifts) and use them as categories. Events of the same type MUST share the exact same category string. If event titles or blocks explicitly mention terms like LECTURE, PRACTICAL, or TUTORIAL, use those as the categories.\n"
        "8. color_id: Google Calendar event colorId (a string representation of integers '1' through '11'). Perform Smart Color Allocation: Map each distinct category/type (e.g. LECTURE, PRACTICAL, TUTORIAL) to a unique and distinct colorId to maximize visual scannability. Ensure LECTURE, PRACTICAL, and TUTORIAL events are assigned different, unique colors.\n\n"
